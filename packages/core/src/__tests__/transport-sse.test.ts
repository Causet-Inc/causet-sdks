...
describe('submitIntentStream', () => {
  it('streams SSE events to callback', async () => {
    ...
    expect(JSON.parse(capturedInit!.body as string).intentType).toBe('T');
  });
  ...
  it('throws on HTTP error', async () => {
    ...
    await expect(submitIntentStream(CFG, {}, () => undefined, fetchImpl)).rejects.toThrow(
      'SSE intent submit failed: 500',
    );
  });
...
describe('openEventSource', () => {
  it('parses JSON message data and forwards to handler', () => {
    const handlers: Array<{ onmessage?: (msg: MessageEvent) => void; onerror?: (e: Event) => void }> = [];
    ...
    handlers[0].onmessage?.({ data: '{"x":1}', lastEventId: '1', type: 'update' } as MessageEvent);
  ...
  it('keeps non-JSON data as string', () => {
    const handlers: Array<{ onmessage?: (msg: MessageEvent) => void }> = [];
    class MockEventSource {
      onmessage: ((msg: MessageEvent) => void) | null = null;
      onerror: ((e: Event) => void) | null = null;
    ...
    handlers[0].onmessage?.({ data: 'plain', lastEventId: '', type: 'message' } as MessageEvent);
    expect(received[0]).toEqual({ id: '', event: 'message', data: 'plain' });
    vi.unstubAllGlobals();
  });

  it('forwards errors to onError callback', () => {
    const handlers: Array<{ onerror?: (e: Event) => void }> = [];
    class MockEventSource {
      onmessage: ((msg: MessageEvent) => void) | null = null;
      onerror: ((e: Event) => void) | null = null;
      constructor(_url: string) {
