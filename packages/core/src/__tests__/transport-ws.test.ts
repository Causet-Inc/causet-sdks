...
function createMockWebSocketClass() {
  const instances: Array<WsHandler & { url: string; sent: string[]; close: () => void }> = [];
...
describe('CausetTransportWebSocket buildUrl', () => {
  it('includes api_key when set', () => {
    ...
    const url = (t as unknown as { buildUrl: () => string }).buildUrl();
...
describe('CausetTransportWebSocket buildHello', () => {
  it('builds hello message shape', () => {
    ...
    const hello = (t as unknown as { buildHello: () => Record<string, unknown> }).buildHello();
    ...
    expect((hello.sdk as { name: string }).name).toBe('causet-sdk-js');
  });
  ...
  it('applies fromCursor to default channels', () => {
    ...
    const hello = (t as unknown as { buildHello: () => { subs: Array<{ from_cursor?: number }> } }).buildHello();
  ...
  it('does not override existing from_cursor on channel', () => {
    ...
    const hello = (t as unknown as { buildHello: () => { subs: Array<{ channel: string; from_cursor?: number }> } }).buildHello();
  ...
  it('uses default ledger and state channels', () => {
    ...
    const hello = (t as unknown as { buildHello: () => { subs: Array<{ channel: string }> } }).buildHello();
