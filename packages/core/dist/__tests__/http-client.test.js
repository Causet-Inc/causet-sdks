...
import { CFG, PREFIX, createMockFetch, emptyResponse, jsonResponse, CFG_NO_FORK } from './helpers.js';
describe('stringifyQueryInput (via runQuery)', () => {
    it('stringifies primitives and collections in query body', async () => {
        expect(stringifyQueryInput({ s: 'x', n: 50, f: 1.5, b: true, g: ['Pop', 'Rock'] })).toEqual({
            s: 'x',
            n: '50',
            f: '1.5',
            b: 'true',
...
describe('emitIntent', () => {
    ...
    it('works without bearer token', async () => {
        ...
        const result = await emitIntent({ ...CFG, bearerToken: undefined }, 's', 'e', 'T', {}, undefined, fetchImpl);
...
describe('runQuery', () => {
    ...
    it('stringifies non-string input and sends pagination', async () => {
        ...
        await runQuery(CFG, 'get_artists_by_genres', { genres: ['Pop', 'Rock'], limit: 50 }, { limit: 25, includeTotal: true, fetchImpl });
    ...
    it('sends offset when no cursor and offset > 0', async () => {
        ...
        await runQuery(CFG, 'search_concerts', { query: 'Future' }, { limit: 30, offset: 30, includeTotal: true, fetchImpl });
    ...
    it('prefers cursor over offset', async () => {
        ...
        await runQuery(CFG, 'search_concerts', { query: 'Future' }, { limit: 30, offset: 999, cursor: 'opaque-next', fetchImpl });
        ...
        const fetchImpl = createMockFetch([
            (url) => {
                if (url.includes('/queries/q1'))
                    return jsonResponse({ slug: 'q1', fields: [] });
                return null;
            },
        ]);
        ...
        const fetchImpl = createMockFetch([
            (url) => {
                if (url.includes('/projections'))
                    return jsonResponse([{ name: 'proj1' }]);
                return null;
            },
        ]);
